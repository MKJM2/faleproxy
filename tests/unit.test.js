...
describe('Yale to Fale replacement logic', () => {
  ...
  test('should handle text that has no Yale references', () => {
    const htmlWithoutYale = `
      ...
      <body>
        <h1>Hello World</h1>
        <p>This is a test page with no (Y)ale references.</p>
      </body>
      </html>
    `;
    ...
    expect(modifiedHtml).toContain('<p>This is a test page with no (Y)ale references.</p>');
  });
  ...
  test('should handle case-insensitive replacements', () => {
    ...
    $('body *').contents().filter(function () {
      ...
      const text = $(this).text();
      const newText = text.replace(/Yale/gi, 'Fale').replace(/yale/g, 'fale').replace(/YALE/g, 'FALE');
      if (text !== newText) {
        $(this).replaceWith(newText);
      }
    });
